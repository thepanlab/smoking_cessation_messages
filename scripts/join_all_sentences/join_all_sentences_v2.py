...
def join_files(config_json):
    ...
    df_all_no_prompts = pd.concat([df_all, df_dataset_subset_columns], ignore_index=True)
    
    df_all_2 = pd.DataFrame()
    df_all_2_no_prompts = pd.concat([df_all, df_dataset_subset_columns], ignore_index=True)
    ...
    for path_csv in config_json["partition_data"]:
        ...
        if "train" in filename:
            df_temp["type"] = "original-train"
        elif "validation" in filename:
            df_temp["type"] = "original-validation"
        df_all_2 = pd.concat([df_all_2, df_temp], ignore_index=True)
        df_all_2_no_prompts = pd.concat([df_all_2_no_prompts, df_temp], ignore_index=True)
