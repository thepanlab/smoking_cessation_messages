...
""" join_results_perplexity.py
    ...
    python join_results_perplexity.py -j \
    ./join_results_perplexity_vB/join_results_perplexity.json
"""
import os
import argparse
import json
...
def get_n_parallel_processes(config_json):
    ...
    if n_parallel_processes == "all":
        ...
        raise ValueError(f"Number of parallel process greater than\
                          the actual number of cores :{num_cores}")
...
def criteria_on_dataframe(df_messages, name, list_criteria):
    ...
    df_messages[name] = \
        df_messages.apply(lambda x: \
                          get_boolean_criteria(x["Message"],
                                               list_criteria), axis=1)
    return df_messages
...
def apply_criteria(config_json):
    ...
    for filepath in config_json["list_filenames"]:
        ...
        for name, list_criteria in zip(config_json["criteria_names"],
                                       config_json["criteria"]):
            df_messages = criteria_on_dataframe(df_messages,
                                                name, list_criteria)
        
        filename = os.path.basename(filepath)
        filepath_output = os.path.join(config_json["path_output"],
                                       f"{filename[:-4]}_criteria_value.csv")
        ...
        filepath_output = os.path.join(config_json["path_output"],
                                       f"{filename[:-4]}_criteria_remaining.csv")
        ...
        for i, criterion_name in enumerate(criteria_names):
            ...
            dict_df_row[f"n_{criterion_name}"] = \
                [df_messages.query(str_query).shape[0]]
