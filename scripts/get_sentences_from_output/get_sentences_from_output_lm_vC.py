""" get_sentences_from_output_lm_vC.py

    It allows to process sentences with vC of prompts

    python get_sentences_from_output_lm_vC.py -j ./get_sentences_from_output_lm_vC/get_sentences_from_output_lm_gpt-j-6B.json
"""
...
def get_file_to_sentences(config_json):
    ...
    for csv_filepath in config_json["list_filenames"]:
        ...
        if "input_v1" in filename or "input_v2" in filename or "input_v3" in filename:
            ...
            for i_row in range(df_generated.shape[0]):
                ...
                for i_regex in range(len(l_start_search) - 1):
                    # Check if the message is empty
                    if l_start_search[i_regex+1] - l_end_search[i_regex] <=1:
                        continue
                    else:
                        # if message is not empty
                        # then extract like this
        ...
        if "input_v5" in filename:
            ...
            for i_row in range(df_generated.shape[0]):
                ...
                # x = re.search(rf"\nMessage [1-9][0-9]?:", df_generated.iloc[i_row,0])
                ...
                for i_regex in range(len(l_start_search_subset) - 1):
                    ...
                    if l_start_search_subset[i_regex+1] - l_end_search_subset[i_regex] <=1:
                        ...
                        if extracted_message[-1] == ":":
                            # create a message until the end
                            # check last character is new line or not
                            if df_generated.iloc[i_row,0][l_start_search[len(l_start_search_subset)-1]] == "\n":
                                extracted_message = df_generated.iloc[i_row,0][l_end_search_subset[i_regex]+1:
                                                                                      l_start_search_subset[len(l_start_search_subset)-1]-1]
                            else:
                                extracted_message = df_generated.iloc[i_row,0][l_end_search_subset[i_regex]+1:
                            ...
                            # if extracted message contains Task, delete from that point
                            # otherwise, do nothing                            
                            
                        x = re.search("Task", extracted_message)
                        if x is not None:
                            extracted_message = extracted_message[:x.start()-2]
